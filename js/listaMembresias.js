document.addEventListener('DOMContentLoaded', () => {
    ...
    const apiUrl = 'https://www.musclegarage.somee.com/Membresias';  // URL del API
    let membresia = [];
    ...
    async function fetchmembresia() {
        try {
            ...
            membresia = await response.json();
            console.log(membresia);
            rendermembresia();
        } catch (error) {
            console.error('Error al consultar las membresias:', error);
        }
    }
    ...
    function rendermembresia() {
        ...
        const filteredmembresia = membresia.filter(product =>
            product.nombre.toLowerCase().includes(searchTerm) || 
            product.duracion.toLowerCase().includes(searchTerm) ||
            product.precio.toLowerCase().includes(searchTerm)
        );
        productList.innerHTML = filteredmembresia.slice(start, end).map(product => `
            <tr>
                <td class="px-6 py-4 whitespace-nowrap">${product.nombre}</td>
                <td class="px-6 py-4 whitespace-nowrap">${product.duracion}</td>
    ...
    searchProductInput.addEventListener('input', rendermembresia);
    ...
    function attachEventListeners() {
        document.querySelectorAll('.edit-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                const product = membresia.find(p => p.nombre === nombre);  // Buscar producto por nombre
            ...
            <div class="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
                <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
                    ...
                    <form id="product-form" class="space-y-4">
                        ...
                        <div>
                            <label for="cantidad" class="block font-medium">Duracion</label>
                            <input type="number" id="cantidad" value="${product.duracion || ''}" class="w-full border border-gray-300 rounded-md p-2">
                        </div>
                        ...
                        <div class="flex justify-end space-x-4">
                            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">${product.nombre ? 'Guardar Cambios' : 'Agregar Membresia'}</button>
    ...
    async function handleAddSubmit(e) {
        ...
        const duracion = document.getElementById('cantidad').value;
        ...
        try {
            ...
            fetchmembresia();  // Refresh list
            window.location.href = 'listaMembresia.html'; // Redirección al visitas
            document.querySelector('.fixed').remove();
        } catch (error) {
    ...
    async function handleEditSubmit(e, nombre) {
        ...
        try {
            ...
            console.error('Error al editar membresia:', error);
        }
    }
    ...
    async function handleDelete(e) {
        const nombre = e.target.dataset.nombre;
        if (confirm('¿Estás seguro de que deseas eliminar esta membresia?')) {
            try {
                ...
                console.error('Error al eliminar el membresia:', error);
            }
        }
    }
